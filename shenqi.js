...
const ROSTER = [
  ...
  // 名将：不进商店，神器认主觉醒；hero 系数更高，觉醒继承星级
  { id: "guanyu",    name: "关羽",   cls: "infantry", cost: 4, hero: true, head: "13",  anim: { me: "126", foe: "126" }, skillName: "青龙偃月斩" },
  { id: "zhangfei",  name: "张飞",   cls: "brawler",  cost: 4, hero: true, head: "40",  anim: { me: "127", foe: "127" }, skillName: "燕人咆哮" },
  { id: "zhaoyun",   name: "赵云",   cls: "cavalry",  cost: 4, hero: true, head: "41",  anim: { me: "128", foe: "128" }, skillName: "七进七出" },
  { id: "zhugeliang",name: "诸葛亮", cls: "priest",   cost: 4, hero: true, head: "42",  anim: { me: "125", foe: "125" }, skillName: "锦囊妙计" },
  { id: "lvbu",      name: "吕布",   cls: "cavalry",  cost: 4, hero: true, head: "126", anim: { me: "135", foe: "135" }, skillName: "无双乱舞" },
];
...
const SYNERGIES = [
  ...
  { key: "hero",     type: "hero", th: [2, 3], desc: ["名将同心：全军攻防+10%", "名将同心：全军攻防+20%"] },
];

// ---------- 装备与神器（图标=原版物品编号）----------
// 神器认主：交给达标的对应兵种 → 觉醒为名将（继承星级和装备）
const ITEMS = {
  ...
  qinglong: { name: "青龙偃月刀", icon: "25", relic: { cls: "infantry", hero: "guanyu" },     desc: "2星步兵拿起它，觉醒为【关羽】" },
  shemao:   { name: "丈八蛇矛",   icon: "26", relic: { cls: "brawler",  hero: "zhangfei" },   desc: "2星武者拿起它，觉醒为【张飞】" },
  qinggang: { name: "青釭剑",     icon: "22", relic: { cls: "cavalry",  hero: "zhaoyun" },    desc: "2星铁骑拿起它，觉醒为【赵云】" },
  shenshan: { name: "五火神焰扇", icon: "34", relic: { cls: "priest",   hero: "zhugeliang" }, desc: "2星方士拿起它，觉醒为【诸葛亮】" },
  fangtian: { name: "方天画戟",   icon: "27", relic: { cls: "cavalry",  hero: "lvbu" },       desc: "2星铁骑拿起它，觉醒为【吕布】" },
};
const RELIC_KEYS = Object.keys(ITEMS).filter(k => ITEMS[k].relic);
...
// ---------- 资源 ----------
const images = {}, effects = {}, itemImgs = {}, heroHeads = {};
...
async function loadAssets() {
  ...
  for (const r of ROSTER) {
    if (r.hero) jobs.push(loadRawImage(`assets/head/${r.head}.png`).then(im => heroHeads[r.id] = im));
  }
...
// ---------- 对局状态 ----------
let phase = "pick";          // pick(选神器) | shop | fight | over
...
function makeUnit(rosterId, side, col, row, star = 1, items = []) {
  ...
  // 名将系数更高（cost4 系数×1.2），基础兵按费用成长
  const m = STAR_MULT[star] * (1 + (R.cost - 1) * 0.18) * (R.hero ? 1.2 : 1);
  const base = {
...
function bakeStats(u, syn) {
  ...
  if (syn) {
    ...
    if (t("hero")) { const v = t("hero") === 1 ? 0.10 : 0.20; atkM += v; defA += t("hero") === 1 ? 2 : 4; }
  ...
  if (u.R.hero) rageM += 0.25;      // 名将怒气获取更快，大招更勤
...
function synergyTiers(sideUnits) {
  const { cls, hero } = synergyCounts(sideUnits);
  const tiers = {};
...
function dealDamage(att, tgt, mult, opt = {}) {
  ...
  gainRage(tgt, 16);
  const isHeroHit = att.R && att.R.hero;
  const label = (counter ? "克制 " : "") + (crit ? "暴击 " : "-") + dmg;
  popup(tgt, label, counter ? "#ffa245" : (crit || isHeroHit) ? "#ffd24a" : "#ff7a5a", counter || crit || isHeroHit);
  if (crit) doShake(3);
  thock(crit ? 110 : 160);
  if (tgt.hp <= 0) {
    ...
    // 名将击杀：斩字特写 + 全场顿帧
    if (isHeroHit) {
      popups.push({ x: tgt.col, y: tgt.row, text: "斩！", color: "#ff4a3a", born: performance.now(), big: true });
...
// 名将专属弹道：关羽刀气 / 诸葛亮火球
function shootSlash(att, tgt, mult) {
  projectiles.push({
    x0: att.col, y0: att.row, x1: tgt.col, y1: tgt.row, type: "slash",
    born: performance.now(), dur: 70 * dist(att, tgt) + 60, done: false,
    onHit: () => { dealDamage(att, tgt, mult); knockback(att, tgt); },
  });
  thock(240, 0.09, 0.25);
}
function shootFire(att, tgt, mult) {
  projectiles.push({
    x0: att.col, y0: att.row, x1: tgt.col, y1: tgt.row, type: "fire",
    born: performance.now(), dur: 95 * dist(att, tgt) + 60, done: false,
    onHit: () => { spawnFx("Meff_4", tgt.col, tgt.row, 0.9); dealDamage(att, tgt, mult); },
  });
  thock(200, 0.08, 0.2);
}
// 名将平A：每一下都有戏
function heroBasic(u, tgt) {
  switch (u.rosterId) {
    case "guanyu":
      shootSlash(u, tgt, 1.15);
      break;
    case "zhangfei":
      dealDamage(u, tgt, 1.15);
      doShake(3);
      spawnFx("Meff_13", tgt.col, tgt.row, 0.9);
      break;
    case "zhaoyun":
      dealDamage(u, tgt, 0.72);
      setTimeout(() => { if (tgt.state !== "dead" && u.state !== "dead") dealDamage(u, tgt, 0.72); }, 160);
      break;
    case "zhugeliang":
      shootFire(u, tgt, 1.1);
      break;
    case "lvbu": {
      dealDamage(u, tgt, 1.0);
      const extra = alive(u.side === "me" ? "foe" : "me")
        .filter(f => f !== tgt && cheby(u, f) <= 1)[0];
      if (extra) dealDamage(u, extra, 0.8);
      break;
    }
    default:
      dealDamage(u, tgt, 1);
...
// ---------- 名将专属大招（觉醒后的质变）----------
let heroCastAnim = null;   // {name, skillName, headImg, born}
function heroCast(u, skillName) {
  heroCastAnim = { name: u.R.name, skillName, headImg: heroHeads[u.rosterId], born: performance.now() };
  popup(u, u.R.name + "【" + skillName + "】", "#ffb830", true);
  playSfx("Se_m_28", 0.6);
}
const HERO_SKILLS = {
  guanyu(u, foes) {          // 青龙偃月斩：横劈一整行
    let bestRow = foes[0].row, bestN = -1;
    for (let r = 0; r < ROWS; r++) {
      const n = foes.filter(f => f.row === r).length;
      if (n > bestN) { bestN = n; bestRow = r; }
    }
    const targets = foes.filter(f => f.row === bestRow);
    if (!targets.length) return false;
    heroCast(u, "青龙偃月斩");
    playSfx("Se_m_04");
    faceTo(u, targets[0].col, targets[0].row);
    u.state = "attack"; u.animStart = performance.now();
    doShake(9);
    sweeps.push({ row: bestRow, born: performance.now(), dur: 520 });   // 横贯整行的大刀光
    for (const f of targets) {
      spawnFx("Meff_3", f.col, f.row, 1.6);
      dealDamage(u, f, 2.0, { noDodge: true });
      knockback(u, f);
    }
    return true;
  },
  zhangfei(u, foes) {        // 燕人咆哮：大范围伤害+全体眩晕
    const targets = foes.filter(f => cheby(u, f) <= 2);
    if (!targets.length) return false;
    heroCast(u, "燕人咆哮");
    u.state = "attack"; u.animStart = performance.now();
    spawnFx("Meff_13", u.col, u.row, 2.4);
    doShake(9);
    for (const f of targets) {
      dealDamage(u, f, 1.3, { noDodge: true });
      if (f.state !== "dead") { f.stun = 1; popup(f, "晕眩", "#e8c8ff"); }
    }
    return true;
  },
  zhaoyun(u, foes) {         // 七进七出：连续冲锋最多3个不同敌人
    if (!foes.length) return false;
    heroCast(u, "七进七出");
    const hit = new Set();
    let dashed = 0;
    for (let n = 0; n < 3; n++) {
      const tgt = alive(u.side === "me" ? "foe" : "me")
        .filter(f => !hit.has(f.uid))
      ...
      for (let s = 0; s < 5 && dist(u, tgt) > 1; s++) {
        const nxt = bfsNextStep(u, goals);
        if (!nxt) break;
        faceTo(u, nxt.c, nxt.r);
        u.col = nxt.c; u.row = nxt.r;
      ...
      if (dist(u, tgt) <= 1) {
        faceTo(u, tgt.col, tgt.row);
        u.state = "attack"; u.animStart = performance.now();
        spawnFx("Meff_13", tgt.col, tgt.row, 1.3);
        dealDamage(u, tgt, 1.5, { noDodge: true });
        knockback(u, tgt);
        dashed++;
      }
    }
  ...
  zhugeliang(u, foes) {      // 锦囊妙计：全军回血 + 天降火雨
    heroCast(u, "锦囊妙计");
    playSfx("Se_m_25");
    u.state = "attack"; u.animStart = performance.now();
    for (const a of alive(u.side)) {
    ...
    const targets = foes.slice().sort(() => Math.random() - 0.5).slice(0, 3);
    targets.forEach((f, i) => {
      setTimeout(() => {
        if (f.state !== "dead" && phase === "fight") {
          spawnFx("Meff_4", f.col, f.row, 1.7);
          dealDamage(u, f, 1.6 * u.skillMult, { trueDmg: true, noDodge: true });
        }
      }, 200 + i * 180);
    });
    doShake(6);
  ...
  lvbu(u, foes) {            // 无双乱舞：周身三连横扫
    const near = () => alive(u.side === "me" ? "foe" : "me").filter(f => cheby(u, f) <= 1);
    if (!near().length) return false;
    heroCast(u, "无双乱舞");
    faceTo(u, near()[0].col, near()[0].row);
    u.state = "attack"; u.animStart = performance.now();
    doShake(9);
    for (let i = 0; i < 3; i++) {
      setTimeout(() => {
        if (u.state === "dead" || phase !== "fight") return;
        const ts = near();
        for (const f of ts) {
          dealDamage(u, f, 0.9, { noDodge: true });
          if (i === 2) knockback(u, f);
        }
        if (ts.length) spawnFx("Meff_3", u.col, u.row, 1.7);
      }, i * 220);
    }
    return true;
  },
...
function unitAct(u) {
  ...
  if (u.rage >= RAGE_MAX) {
    const heroFn = u.R.hero && HERO_SKILLS[u.rosterId];
    if (heroFn) {
      if (heroFn(u, foes)) { u.rage = 0; return "heroskill"; }
    } else if (SKILLS[u.cls](u, foes)) { u.rage = 0; return "skill"; }
  }
  if (u.heal) {
...
function endCombat(win) {
  ...
  // 掉落：每逢3的倍数回合，无论胜负都有（神器优先，未拥有且未觉醒的）
  if (round % 3 === 0) {
    const owned = new Set(inventory);
    const relics = RELIC_KEYS.filter(k =>
      !owned.has(k) &&
      !units.some(v => v.side === "me" && v.rosterId === ITEMS[k].relic.hero) &&
      !bench.some(v => v && v.rosterId === ITEMS[k].relic.hero));
    let k;
    if (relics.length && (round <= 9 || Math.random() < 0.55)) {
      k = relics[Math.floor(Math.random() * relics.length)];
      lines.push(`⚡ ${win ? "缴获神器" : "败军中拾得神器"}：<b style="color:#f0c060">${ITEMS[k].name}</b><br><span style="font-size:12px">${ITEMS[k].desc}</span>`);
    } else {
      const normals = Object.keys(ITEMS).filter(x => !ITEMS[x].relic);
      k = normals[Math.floor(Math.random() * normals.length)];
...
function spawnEnemies() {
  ...
  // 中后期敌军也会请出名将
  if (round >= 7 && Math.random() < 0.45 && cells.length) {
    const heroes = ROSTER.filter(r => r.hero);
    const R = heroes[Math.floor(Math.random() * heroes.length)];
    const [c, r] = cells.pop();
    units.push(makeUnit(R.id, "foe", c, r, round >= 12 ? 2 : 1));
  }
}
...
function tryEquip(u) {
  ...
  // 神器认主 → 觉醒
  if (it.relic) {
    if (u.R.hero) { setStatus(`${u.R.name}已是名将，神器另择良主`); return true; }
    if (u.cls !== it.relic.cls) { setStatus(`${it.name} 只认${CLASSES[it.relic.cls].name}`); return true; }
    if (u.star < 2) { setStatus(`需要 2 星${CLASSES[u.cls].name}才能驾驭 ${it.name}`); return true; }
    if (units.some(v => v.side === "me" && v.rosterId === it.relic.hero) ||
        bench.some(v => v && v.rosterId === it.relic.hero)) {
      setStatus(`${R_MAP[it.relic.hero].name}已在阵中`); return true;
    }
    inventory.splice(selItem, 1);
    selItem = -1;
    awaken(u, it.relic.hero, k);
    return true;
  }
...
function showRelicPick() {
  const b = document.getElementById("banner");
  document.getElementById("bannerText").textContent = "择 器";
  document.getElementById("bannerText").style.color = "#f0c060";
  document.getElementById("bannerSub").innerHTML = "选一件开局神器——交给 2 星对应兵种即可觉醒名将<br><span style='font-size:12px'>之后每 3 回合还会获得新的神器</span>";
  document.getElementById("next").style.display = "none";
  const pick = document.getElementById("relicPick");
  pick.innerHTML = "";
  const keys = RELIC_KEYS.slice().sort(() => Math.random() - 0.5).slice(0, 3);
  for (const key of keys) {
    const it = ITEMS[key];
    const hero = R_MAP[it.relic.hero];
    const el = document.createElement("div");
    el.className = "relicCard";
    el.innerHTML = `<img src="assets/items/${it.icon}.png">
      <b>${it.name}</b>
      <span>2星${CLASSES[it.relic.cls].name}觉醒为<br><b style="font-size:13px">${hero.name}</b></span>`;
    el.onclick = () => {
      inventory.push(key);
      ...
      pick.innerHTML = "";
      phase = "shop";
      setStatus(`已获得【${it.name}】：练一个2星${CLASSES[it.relic.cls].name}让它认主`);
      renderInv(); refreshStats(); renderShop();
      playSfx("Se_m_25", 0.5);
    };
    pick.appendChild(el);
  }
...
// ---------- 觉醒 ----------
let awakeAnim = null;   // {name, relicName, headImg, born}
function awaken(u, heroId, relicKey) {
  const R = R_MAP[heroId];
  const nu = makeUnit(heroId, "me", u.col, u.row, u.star, u.items);
  const bi = bench.indexOf(u);
  if (bi >= 0) bench[bi] = nu;
  else { units = units.filter(v => v !== u); units.push(nu); }
  if (selected === u) selected = null;
  if (nu.col !== null) {
    spawnFx("Meff_13", nu.col, nu.row, 1.8);
    spawnFx("Meff_5", nu.col, nu.row, 1.5);
  }
  awakeAnim = { name: R.name, relicName: ITEMS[relicKey].name, headImg: heroHeads[heroId], born: performance.now() };
  playSfx("Se_m_28", 0.7);
  doShake(6);
  setStatus(`${ITEMS[relicKey].name} 认主，${R.name} 觉醒！`);
  renderInv(); refreshStats(); renderSyn();
}
// 名将大招横幅演出（暗幕+头像+技能名，约0.9秒）
function drawHeroCast(now) {
  ...
  // 中部金边横幅带
  const cy = ROWS * TILE * 0.36;
  ctx.globalAlpha = Math.max(0, a * 0.92);
  ctx.fillStyle = "rgba(30,20,8,.95)";
  ctx.fillRect(0, cy - 46, cv.width, 92);
  ctx.fillStyle = "#f0c060";
  ctx.fillRect(0, cy - 48, cv.width, 3);
  ctx.fillRect(0, cy + 45, cv.width, 3);
  const im = heroCastAnim.headImg;
  if (im) {
    ctx.drawImage(im, 26, cy - 38, 61, 76);
    ctx.strokeStyle = "#f0c060"; ctx.lineWidth = 2;
    ctx.strokeRect(26, cy - 38, 61, 76);
  }
  ctx.textAlign = "left";
  ctx.font = "13px sans-serif";
  ctx.fillStyle = "#d8c8a0";
  ctx.fillText(heroCastAnim.name, 102, cy - 16);
  ctx.font = "bold 27px sans-serif";
  ctx.fillStyle = "#ffd24a";
  ctx.shadowColor = "#f0c060"; ctx.shadowBlur = 16;
  ctx.fillText(heroCastAnim.skillName, 102, cy + 20);
  ctx.shadowBlur = 0;
  ctx.restore();
}

// 觉醒全屏演出（金光+头像+大字，约1.8秒）
function drawAwaken(now) {
  if (!awakeAnim) return;
  const el = now - awakeAnim.born;
  if (el > 1800) { awakeAnim = null; return; }
  const a = el < 300 ? el / 300 : el > 1400 ? (1800 - el) / 400 : 1;
  ctx.save();
  ...
  for (let i = 0; i < 12; i++) {
    const ang = (i / 12) * Math.PI * 2 + el / 900;
    ctx.strokeStyle = "rgba(240,192,96," + 0.35 * a + ")";
    ctx.lineWidth = 10;
    ctx.beginPath();
  ...
  // 头像
  const im = awakeAnim.headImg;
  if (im) {
    const scale = Math.min(2.6, 1.4 + el / 400);
    const w = 64 * scale, h = 80 * scale;
    ctx.drawImage(im, cx - w / 2, cy - h / 2, w, h);
    ctx.strokeStyle = "#f0c060"; ctx.lineWidth = 3;
    ctx.strokeRect(cx - w / 2, cy - h / 2, w, h);
  }
  ctx.textAlign = "center";
  ctx.font = "16px sans-serif";
  ctx.fillStyle = "#d8c8a0";
  ctx.fillText(awakeAnim.relicName + " · 认主", cx, cy + 130);
  ctx.font = "bold 40px sans-serif";
  ctx.fillStyle = "#f0c060";
  ctx.shadowColor = "#f0c060"; ctx.shadowBlur = 24;
  ctx.fillText(awakeAnim.name + " 觉醒！", cx, cy + 180);
  ctx.shadowBlur = 0;
  ctx.textAlign = "left";
  ctx.restore();
}
...
function drawUnitAt(u, px, py, now) {
  ...
  const scale = (1.15 + (u.star - 1) * 0.12) * (u.R.hero ? 1.32 : 1);   // 名将体型大一圈
  const flip = u.dir === "right";
  const whiten = now < u.flashUntil;

  // 名将常驻金色气场（光环 + 环绕光点）
  if (u.R.hero && u.state !== "dead") {
    const gy = py + SPR * scale * 0.36;
    const g = ctx.createRadialGradient(px, gy, 4, px, gy, 28);
    g.addColorStop(0, "rgba(255,210,74,.45)");
    g.addColorStop(1, "rgba(255,210,74,0)");
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.ellipse(px, gy, 28, 12, 0, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "rgba(255,210,74,.55)"; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.ellipse(px, gy, 24, 10, 0, 0, Math.PI * 2); ctx.stroke();
    for (let i = 0; i < 3; i++) {
      const ang = now / 480 + i * Math.PI * 2 / 3;
      ctx.fillStyle = "rgba(255,225,130,.9)";
      ctx.beginPath();
      ctx.arc(px + Math.cos(ang) * 24, gy + Math.sin(ang) * 10, 2.2, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ...
  // 手持神器时：可觉醒的棋子金光脉冲提示
  if (phase === "shop" && selItem >= 0 && u.side === "me") {
    const it = ITEMS[inventory[selItem]];
    if (it && it.relic && !u.R.hero && u.cls === it.relic.cls && u.star >= 2) {
      ctx.strokeStyle = "rgba(255,210,74," + (0.55 + 0.4 * Math.sin(now / 130)) + ")";
      ctx.lineWidth = 3;
      ctx.strokeRect(px - TILE / 2 + 1, py - TILE / 2 + 1, TILE - 2, TILE - 2);
    }
  }
  // 名将：脚下金字名号
  if (u.R.hero && u.state !== "dead") {
    ...
    ctx.fillText(u.R.name, px + 1, py + SPR * 0.85 + 1);
    ctx.fillStyle = "#ffd24a";
    ctx.fillText(u.R.name, px, py + SPR * 0.85);
    ctx.textAlign = "left";
  }
  // 当前行动者：脚下白圈
...
function drawProjectiles(now) {
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    if (p.type === "slash") {          // 关羽刀气：青白色大弧光
      ctx.globalAlpha = 0.95;
      ctx.strokeStyle = "rgba(150,255,210,.9)"; ctx.lineWidth = 5;
      ctx.beginPath(); ctx.arc(-6, 0, 22, -1.15, 1.15); ctx.stroke();
      ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 2.2;
      ctx.beginPath(); ctx.arc(-6, 0, 22, -0.95, 0.95); ctx.stroke();
      ctx.strokeStyle = "rgba(150,255,210,.4)"; ctx.lineWidth = 8;
      ctx.beginPath(); ctx.arc(-14, 0, 20, -0.8, 0.8); ctx.stroke();
    } else if (p.type === "fire") {    // 诸葛亮火球：橙色光球+尾焰
      const grad = ctx.createRadialGradient(0, 0, 1, 0, 0, 10);
      grad.addColorStop(0, "#fff0b0"); grad.addColorStop(0.5, "#ff9040"); grad.addColorStop(1, "rgba(255,80,20,0)");
      ctx.fillStyle = grad;
      ctx.beginPath(); ctx.arc(0, 0, 10, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "rgba(255,120,40,.45)";
      ctx.beginPath(); ctx.ellipse(-12, 0, 12, 4, 0, 0, Math.PI * 2); ctx.fill();
    } else {                            // 普通箭矢
...
function drawSweeps(now) {
  for (let i = sweeps.length - 1; i >= 0; i--) {
    ...
    const x = t * (cv.width + 120) - 60;
    const y = s.row * TILE + TILE / 2;
    ctx.save();
    ctx.globalAlpha = 0.9 * (1 - t * 0.35);
    ctx.strokeStyle = "rgba(150,255,210,.85)"; ctx.lineWidth = 7;
    ctx.beginPath(); ctx.arc(x - 26, y, 36, -1.15, 1.15); ctx.stroke();
    ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.arc(x - 26, y, 36, -0.95, 0.95); ctx.stroke();
    ctx.strokeStyle = "rgba(150,255,210,.35)"; ctx.lineWidth = 12;
    ctx.beginPath(); ctx.arc(x - 44, y, 32, -0.75, 0.75); ctx.stroke();
    ctx.restore();
  }
}
