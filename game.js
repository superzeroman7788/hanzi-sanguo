...
function endCombat(win) {
  ...
  // 掉落：3的倍数回合胜利。神器优先（未拥有且未觉醒的），之后混合普通装备
  if (win && round % 3 === 0) {
    ...
    let k;
    if (relics.length && (round <= 6 || Math.random() < 0.55)) {
      k = relics[Math.floor(Math.random() * relics.length)];
      lines.push(`⚡ 缴获神器：<b style="color:#f0c060">${ITEMS[k].name}</b><br><span style="font-size:12px">${ITEMS[k].desc}</span>`);
    } else {
