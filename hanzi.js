...
function nextRound() {
  ...
  // 直接开打：下一关无备战
  phase = "fight"; breachCount = 0; lostCol = -1;
...
function commitPath(path) {
  ...
  if (path.length === 1) {
    ...
    checkMerge("" + C.id, 1);
    {
      const spot = bench[slot] && autoDeploy(bench[slot]);
      if (spot) { spendFx(C.cost, spot.col, spot.row); setStatus(`${CLASSES[C.cls].name}已自动上阵第${spot.col + 1}路（可拖动调整）`); }
      else setStatus(`${CLASSES[C.cls].name}已入备战席`);
    }
  ...
  if (cells.every(x => x.type === "class" && x.clsId === cells[0].clsId) && path.length <= 3) {
    ...
    {
      const spot = bench[slot] && autoDeploy(bench[slot]);
      if (spot) { spendFx(cost, spot.col, spot.row); setStatus(`${star}星${CLASSES[C.cls].name}已上阵第${spot.col + 1}路`); }
    }
  ...
  if (h) {
    ...
    {
      const spot = bench[slot] && autoDeploy(bench[slot]);
      if (spot) spendFx(hp, spot.col, spot.row);
    }
