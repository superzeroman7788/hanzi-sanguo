...
// ========== 兵马版：原版小人帧动画 ==========
// 帧序：移动条48px f0-1下 f2-3上 f4-5左 f6/7/8站(下/上/左) f9-10亡；攻击条64px a0-3下 a4-7上 a8-11左；右=左镜像
...
// 角色映射：我方按兵种(player色)、敌方按敌种(enemy色)、名将/曹将专属
const ANIM_ME  = { infantry: "4", lancer: "7", archer: "19", priest: "82", cavalry: "10" };
const ANIM_FOE = { zu: "3", ma: "102", dun: "6", qi2: "66", nu: "24", xiao: "54" };
const ANIM_HERO = {
  guanyu: "126", zhangfei: "127", zhaoyun: "128", zhugeliang: "125", lvbu: "135",
  liubei: "110", huangzhong: "25", huanggai: "97", guanping: "1", zhangliao: "13",
  xiahoudun: "9", xiahouyuan: "27", caoren: "39", caohong: "12", xuchu: "45",
  dianwei: "42", xuhuang: "9", zhanghe: "15", simayi: "72", caocao: "124",
};
function unitAnimId(u) {
  if (u.hero) return ANIM_HERO[u.defId] || "110";
  if (u.side === "foe") return ANIM_FOE[(u.defId || "").replace("f_", "")] || "3";
  return ANIM_ME[u.cls] || "4";
}
const sprImages = {};
(function loadSprites() {
  const ids = new Set([...Object.values(ANIM_ME), ...Object.values(ANIM_FOE), ...Object.values(ANIM_HERO)]);
  for (const id of ids) {
    loadKeyedImage(`assets/move/${id}.png`).then(im => sprImages[`m${id}`] = im).catch(() => {});
    loadKeyedImage(`assets/attack/${id}.png`).then(im => sprImages[`a${id}`] = im).catch(() => {});
  }
})();
...
function drawTile(u, px, py, now, sizeBase) {
  return drawTileSprite(u, px, py, now, sizeBase);
}
function drawTileSprite(u, px, py, now, sizeBase) {
  const isBench = !!sizeBase;
  const scale = (isBench ? 0.82 : 1.18 + (u.star - 1) * 0.12) * (u.hero ? 1.3 : 1);
  const flip = u.dir === "right";
  const whiten = now < u.flashUntil;
  const id = unitAnimId(u);
  ...
  if (u.state === "attack") {
    ...
    ox += P.dx * dirX * 0.7; oy += P.dx * dirY * 0.7;
  }
  // 受击晃动
  if (u.hitAnim) {
    const hel = now - u.hitAnim.born;
    if (hel > 370) u.hitAnim = null;
    else { const P = poseAt(KF_HIT, hel); oy += P.dy * u.hitAnim.dir * 0.8; }
  }
  ...
  if (u.hero && u.state !== "dead") {
    ...
    ctx.save();
    ctx.strokeStyle = "rgba(240,200,96," + (0.5 + 0.3 * Math.sin(now / 300)) + ")";
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(cx2, gy, 24 * scale * 0.8, 9 * scale * 0.8, 0, 0, 7); ctx.stroke();
    ctx.restore();
  }
  // 死亡：原版阵亡帧淡出
  if (u.state === "dead") {
    const el = now - u.deadAt;
    const fade = Math.max(0, 1 - el / 1400);
    if (fade <= 0) return;
    blitFrame(sprImages[`m${id}`], el < 260 ? DEAD_FRAMES[0] : DEAD_FRAMES[1], cx2, cy2, SPR * scale, false, fade);
    return;
  }
  // 本体帧
  if (u.state === "attack") {
    const el = now - u.animStart;
    const fs = ATK_FRAMES[u.dir] || ATK_FRAMES.down;
    const fi = Math.min(fs.length - 1, Math.max(0, Math.floor(el / 110)));
    blitFrame(sprImages[`a${id}`], fs[fi], cx2, cy2, ATK_SPR * scale, flip, undefined, whiten);
    if (el > 540) u.state = "stand";
  } else if (u.state === "walk") {
    const fs = MOVE_FRAMES[u.dir] || MOVE_FRAMES.down;
    blitFrame(sprImages[`m${id}`], fs[Math.floor(now / 200) % fs.length], cx2, cy2, SPR * scale, flip, undefined, whiten);
  } else {
    blitFrame(sprImages[`m${id}`], (STAND_FRAMES[u.dir] != null ? STAND_FRAMES[u.dir] : 6), cx2, cy2, SPR * scale, flip, undefined, whiten);
  }
  // 选中金框 / 行动白圈
  if (selected === u) {
    ctx.strokeStyle = "#b8891c"; ctx.lineWidth = 2.5;
    ctx.strokeRect(cx2 - TILE / 2 + 4, cy2 - TILE / 2 + 4, TILE - 8, TILE - 8);
  }
  if (phase === "fight" && u.actingUntil && now < u.actingUntil) {
    ctx.strokeStyle = "rgba(255,252,240,.8)"; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(cx2, cy2 + SPR * scale * 0.38, 20, 8, 0, 0, 7); ctx.stroke();
  }
  // 名将脚下名号
  if (u.hero) {
    ctx.font = 'bold 11px sans-serif'; ctx.textAlign = "center";
    ctx.fillStyle = "rgba(0,0,0,.7)";
    ctx.fillText(u.name, cx2 + 1, cy2 + SPR * scale * 0.68 + 1);
    ctx.fillStyle = "#ffd24a";
    ctx.fillText(u.name, cx2, cy2 + SPR * scale * 0.68);
    ctx.textAlign = "left";
  }
  // 血条+怒气+星级
  const w = isBench ? 30 : 40, bx = cx2 - w / 2, by = cy2 - SPR * scale / 2 - 10;
  ...
  if (u.star > 1) {
    ...
    ctx.fillText("★".repeat(u.star), cx2, by - 3);
    ctx.textAlign = "left";
  }
  if (u.stun > 0) {
    ctx.fillStyle = "#8a5aa0"; ctx.font = "11px sans-serif"; ctx.textAlign = "center";
    ctx.fillText("✦晕", cx2, by - 13);
    ctx.textAlign = "left";
  }
  return;
}
function drawTileLegacy(u, px, py, now, sizeBase) {
